    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    async fn _documents_batch_update(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    "write-control.required-revision-id" => Some(("writeControl.requiredRevisionId", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    "write-control.target-revision-id" => Some(("writeControl.targetRevisionId", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["required-revision-id", "target-revision-id", "write-control"]);
        ...
        let mut request: api::BatchUpdateDocumentRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.documents().batch_update(request, opt.value_of("document-id").unwrap_or(""));
