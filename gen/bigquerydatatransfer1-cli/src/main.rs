...
use google_bigquerydatatransfer1::{api, Error, oauth2, client::chrono, FieldMask};
...
struct Engine<'n, S> {
    opt: ArgMatches<'n>,
    hub: api::BigQueryDataTransfer<S>,
    gp: Vec<&'static str>,
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn _projects_enroll_data_sources(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::EnrollDataSourcesRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().enroll_data_sources(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_enroll_data_sources(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::EnrollDataSourcesRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().locations_enroll_data_sources(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    "owner-info.email" => Some(("ownerInfo.email", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("enroll-data-sources", Some(opt)) => {
                        call_result = self._projects_enroll_data_sources(opt, dry_run, &mut err).await;
                    },
                    ...
                    ("locations-enroll-data-sources", Some(opt)) => {
                        call_result = self._projects_locations_enroll_data_sources(opt, dry_run, &mut err).await;
                    },
