...
use google_versionhistory1::{api, Error, oauth2, client::chrono, FieldMask};
...
{
    async fn _platforms_channels_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.platforms().channels_list(opt.value_of("parent").unwrap_or(""));
    ...
    async fn _platforms_channels_versions_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.platforms().channels_versions_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "page-token" => {
    ...
    async fn _platforms_channels_versions_releases_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.platforms().channels_versions_releases_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "page-token" => {
    ...
    async fn _platforms_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.platforms().list(opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "page-token" => {
            ...
            ("platforms", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("channels-versions-list", Some(opt)) => {
                        call_result = self._platforms_channels_versions_list(opt, dry_run, &mut err).await;
                    },
                    ("channels-versions-releases-list", Some(opt)) => {
                        call_result = self._platforms_channels_versions_releases_list(opt, dry_run, &mut err).await;
                    },
                    ("list", Some(opt)) => {
                        call_result = self._platforms_list(opt, dry_run, &mut err).await;
                    },
                    _ => {
                        err.issues.push(CLIError::MissingMethodError("platforms".to_string()));
                        writeln!(io::stderr(), "{}\n", opt.usage()).ok();
                    }
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>, connector: S) -> Result<Engine<'n, S>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "versionhistory1-secret.json",
        ...
        let auth = oauth2::InstalledFlowAuthenticator::with_client(
            ...
            client.clone(),
        ).persist_tokens_to_disk(format!("{}/versionhistory1", config_dir)).build().await.unwrap();
