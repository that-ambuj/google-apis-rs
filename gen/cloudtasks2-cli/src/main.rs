...
use google_cloudtasks2::{api, Error, oauth2, client::chrono, FieldMask};
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["app-engine-routing-override", "audience", "host", "http-method", "http-target", "instance", "max-attempts", "max-backoff", "max-burst-size", "max-concurrent-dispatches", "max-dispatches-per-second", "max-doublings", "max-retry-duration", "min-backoff", "name", "oauth-token", "oidc-token", "path", "path-override", "port", "purge-time", "query-override", "query-params", "rate-limits", "retry-config", "sampling-ratio", "scheme", "scope", "service", "service-account-email", "stackdriver-logging-config", "state", "uri-override", "uri-override-enforce-mode", "version"]);
